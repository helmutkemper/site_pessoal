...
type HtmlInputEmail struct {
	...
	*/
	Multiple Boolean `htmlAttr:"multiple"`
...
func (el *HtmlInputEmail) ToHtml() []byte {
	...
	buffer.Write([]byte(`<input type="email"`))
...
func (el *HtmlInputEmail) GetId() []byte {
	if el.Global.Id == "" {
		el.Global.Id = GetAutoId()
	}
	return []byte(el.Global.Id)
}
func (el *HtmlInputEmail) GetName() []byte {
	if el.Name == "" {
		el.Name = GetAutoId()
	}
	return []byte(el.Name)
}
func (el *HtmlInputEmail) ToJavaScript() []byte {
