...
// <input> elements of type "range" let the user specify a numeric value which must be no less than a given value, and
// no more than another given value. The precise value, however, is not considered important. This is typically
// represented using a slider or dial control rather than a text entry box like the "number" input type. Because this
// kind of widget is imprecise, it shouldn't typically be used unless the control's exact value isn't important.
//
// <input type="range">
type HtmlInputRange struct {
	/*
...
func (el *HtmlInputRange) ToHtml() []byte {
	...
	buffer.Write([]byte(`<input type="range"`))
...
func (el *HtmlInputRange) GetId() []byte {
	if el.Global.Id == "" {
		el.Global.Id = GetAutoId()
	}
	return []byte(el.Global.Id)
}
func (el *HtmlInputRange) GetName() []byte {
	if el.Name == "" {
		el.Name = GetAutoId()
	}
	return []byte(el.Name)
